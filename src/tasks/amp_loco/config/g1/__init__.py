...
# STATE ORACLE + WALL: same ground-truth ball-state perception regime, but a static
# wall stands beside the spawn point (randomized side/distance each episode). The
# robot must dodge the ball without crashing into the wall: wall state is added to
# the symmetric ball_state obs group, a per-link static-barrier reward shapes wall
# clearance, and wall contact terminates with the -200 is_terminated penalty.
register_mjlab_task(
  task_id="Unitree-G1-AMP-Dodge-MimicKit-Wall-Flat",
  env_cfg=g1_amp_dodge_mimickit_wall_flat_env_cfg(),
  play_env_cfg=g1_amp_dodge_mimickit_wall_flat_env_cfg(play=True),
  rl_cfg=g1_amp_dodge_mimickit_ppo_runner_cfg(),
  runner_cls=AMPOnPolicyRunner,
)
